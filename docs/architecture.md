...
Admin analytics reads are exposed through `GET /wp-json/librefunnels/v1/analytics/summary` behind `manage_woocommerce`. The first summary returns event counts, attributed revenue, store currency, order count, and offer accept rate for a selected period and optional funnel ID. The React canvas now reads this local endpoint for the selected funnel and renders a compact last-30-days panel with beginner-friendly empty guidance. Future dashboard UI should continue reading this endpoint instead of scanning WooCommerce orders or calling external analytics services.
